...
!include ".\language_${LANGUAGE_CODE}.nsh"
...
Function DUIPage
	...
	#设置安装包的标题及任务栏显示
	nsNiuniuSkin::SetWindowTile $hInstallDlg ""
	nsNiuniuSkin::ShowPageItem $hInstallDlg "wizardTab" ${INSTALL_PAGE_CONFIG}
	nsNiuniuSkin::SetControlAttribute $hInstallDlg "need_space" "text" "${LANG_INSTALL_SPACE} ${APPLICATION_SIZE} M"
	nsNiuniuSkin::SetControlAttribute $hInstallDlg "installLabel" "text" "${LANG_INSTALL_PATH}"
	nsNiuniuSkin::SetControlAttribute $hInstallDlg "btnInstall" "text" "${LANG_INSTALL}"
	nsNiuniuSkin::SetControlAttribute $hInstallDlg "progress_tip" "text" "${LANG_INSTALLING}"
	nsNiuniuSkin::SetControlAttribute $hInstallDlg "btnRun" "text" "${LANG_START}"
...
Function un.DUIPage
	...
	#设置安装包的标题及任务栏显示
	nsNiuniuSkin::SetWindowTile $hInstallDlg "${PRODUCT_NAME}${LANG_UNINSTALL}"
	nsNiuniuSkin::SetWindowSize $hInstallDlg 480 390
	Call un.BindUnInstUIControls


	nsNiuniuSkin::SetControlAttribute $hInstallDlg "sure_uninstall" "text" "${LANG_UNINSTALL_WARNING}"
	nsNiuniuSkin::SetControlAttribute $hInstallDlg "btnUnInstall" "text" "${LANG_SURE}"
	nsNiuniuSkin::SetControlAttribute $hInstallDlg "btnCancel" "text" "${LANG_CANCEL}"
	nsNiuniuSkin::SetControlAttribute $hInstallDlg "un_progress_tip" "text" "${LANG_UNINSTALLING}"
	nsNiuniuSkin::SetControlAttribute $hInstallDlg "btnUninstalled" "text" "${LANG_COMPLETE}"
	nsNiuniuSkin::SetControlAttribute $hInstallDlg "uninstalled" "text" "${LANG_UNINSTALLED}"
...
Function un.BindUnInstUIControls
	...
	GetFunctionAddress $0 un.ExitDUISetup
    nsNiuniuSkin::BindCallBack $hInstallDlg "btnCancel" $0
FunctionEnd
...
Function OnRichEditTextChange
	...
	${If} $R5 == "0"
		nsNiuniuSkin::SetControlAttribute $hInstallDlg "local_space" "text" "${LANG_ILLEGALPATH}"
	...
	${If} $R0 > 1024                               #400即程序安装后需要占用的实际空间，单位：MB
		...
		IntOp $R1  $R0 % 1024
		IntOp $R0  $R0 / 1024;
		nsNiuniuSkin::SetControlAttribute $hInstallDlg "local_space" "text" "${LANG_INSTALL_LEFT_SPACE}$R0.$R1GB"
	${Else}
		nsNiuniuSkin::SetControlAttribute $hInstallDlg "local_space" "text" "${LANG_INSTALL_LEFT_SPACE}$R0.$R1MB"
     ${endif}
...
Function ShowMsgBox
	nsNiuniuSkin::InitSkinSubPage "msgBox.xml" "btnOK" "msgBtnCancel,msgBtnClose"  ; "提示" "${PRODUCT_NAME} 正在运行，请退出后重试!" 0
	Pop $hInstallSubDlg
	nsNiuniuSkin::SetControlAttribute $hInstallSubDlg "lblTitle" "text" ""
	nsNiuniuSkin::SetControlAttribute $hInstallSubDlg "lblMsg" "text" "$R8"
	nsNiuniuSkin::SetControlAttribute $hInstallSubDlg "btnOK" "text" "${LANG_SURE}"
	nsNiuniuSkin::SetControlAttribute $hInstallSubDlg "msgBtnCancel" "text" "${LANG_CANCEL}"
	nsNiuniuSkin::SetControlAttribute $hInstallSubDlg "msgBtnClose" "text" "${LANG_CANCEL}"
	${If} "$R7" == "1"
...
Function OnBtnInstall
	...
	${If} $R0 == 0
        StrCpy $R8 "${PRODUCT_NAME} ${LANG_WARINING_RUNNING}"
		StrCpy $R7 "0"
		Call ShowMsgBox
		goto InstallAbort
    ${EndIf}
	...
	Call IsSetupPathIlleagal
	${If} $R5 == "0"
		StrCpy $R8 "${LANG_WARINING_ILLEGALPATH}"
		StrCpy $R7 "0"
		Call ShowMsgBox
		goto InstallAbort
    ${EndIf}
	${If} $R5 == "-1"
		StrCpy $R8 "${LANG_WARINING_SPACE}"
		StrCpy $R7 "0"
		Call ShowMsgBox
		goto InstallAbort
    ${EndIf}
	...
	#${If} $R0 == "1"
		...
		CreateShortCut "$DESKTOP\${LANG_APP_NAME}.lnk" "$INSTDIR\${EXE_NAME}"
...
Function OnBtnSelectDir
    nsNiuniuSkin::SelectInstallDirEx $hInstallDlg "${LANG_SELECT_PATH}"
    Pop $0
	${Unless} "$0" == ""
...
Function un.ShowMsgBox
	nsNiuniuSkin::InitSkinSubPage "msgBox.xml" "btnOK" "msgBtnCancel,msgBtnClose"  ; "提示" "${PRODUCT_NAME} ${LANG_WARINING_RUNNING}" 0
	Pop $hInstallSubDlg
	nsNiuniuSkin::SetControlAttribute $hInstallSubDlg "lblTitle" "text" "${LANG_INFO}"
	nsNiuniuSkin::SetControlAttribute $hInstallSubDlg "lblMsg" "text" "$R8"
	nsNiuniuSkin::SetControlAttribute $hInstallSubDlg "msgBtnCancel" "text" "${LANG_CANCEL}"
	${If} "$R7" == "1"
		nsNiuniuSkin::SetControlAttribute $hInstallSubDlg "hlCancel" "visible" "true"
...
Function un.onUninstall
	...
	#此处检测当前是否有程序正在运行，如果正在运行，提示先卸载再安装
	nsProcess::_KillProcess "${PRODUCT_NAME} Setup ${PRODUCT_VERSION}.exe"
    nsExec::Exec "taskkill /im ${PRODUCT_NAME}*"
	...
	RMDir /r "$instdir"
	RMDir /r "$profile\AppData\Roaming\pbrmax_client"
